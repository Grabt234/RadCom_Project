...
%file name
hdf5_file_name_emission = "c_emission.h5"
hdf5_file_name_response = "c_response.h5"
...
slow_time_samples = i/coherently;

%preallocating memory
range_response = zeros(slow_time_samples,length(conv(matched_filter,slow_time(1,:))));


for j = 1:slow_time_samples
    
    for k = 1:coherently
        
        range_response(j,:) = range_response(j,:) + conv(matched_filter,slow_time((j-1)*coherently+k,:));
