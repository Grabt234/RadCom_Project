function iq_data_raw = iq_read(file_name, file_datatype, ...
    ...
    % ---------------------------------------------------------------------    
    % read_data: Reads in complex (IQ) data from a binary file
    % ---------------------------------------------------------------------
    % Usage:
    %  Inputs
    ...
    if (file_datatype == "short")
        % 4 bytes per short (x2 for cmplx)
        fseek(file_id, 2*4*file_offset, 'bof'); 
    elseif (file_datatype == "double")
        % 8 bytes per double (x2 for cmplx)
        fseek(file_id, 2*8*file_offset, 'bof');  
    end
